...
#ifdef OPUS_X64
LRESULT CALLBACK FedtWndProc(HWND hwnd, UINT wm, WPARAM wParam, LPARAM lParam)
#else
...
int fn;
{
	...
	lRet = SendMessage(hwndParent, WM_COMMAND,
			GetDlgCtrlID(hwnd), MakeLong(hwnd, fn));
...
#ifdef OPUS_X64
	SetWindowLongPtr(hwnd, 0, (LONG_PTR)hfedt);
#else
	SetWindowWord(hwnd, 0, hfedt);
#endif
	pfedtT->liMax = 1;
	pfedtT->hwnd = hwnd;
	pfedtT->hwndParent = GetParent(hwnd);
...
#ifdef OPUS_X64
			((hbr = (HBRUSH)SendMessage(hwndParent, WM_CTLCOLOREDIT,
			(WPARAM)hdc, (LPARAM)pfedt->hwnd)) != NULL))
#else
