...
#if defined(DEBUG) || defined(OPUS_X64) /* in editn.asm on Win16 */
/* P X S  I N I T */
...
struct XBC *pxbc;
{
	...
	for (ptn = pxbc->rgtn, ptnMac = ptn + pxbc->itnMac; ptn < ptnMac; ptn++)
		...
		case tntDelBkmk:
				...
				{
				struct DOD *pdod = PdodDoc((int)(intptr_t)ptn->h/*doc*/);
				FreezeHp();
				Assert(!pdod->fShort);
...
#ifdef OPUS_X64
	hplcDest = *(struct PLC ***)((CHAR *)(*hdodDest) + edc * sizeof(int));
	hplcSrc = *(struct PLC ***)((CHAR *)PdodDoc(pcaSrc->doc) +
			edc * sizeof(int));
#else
