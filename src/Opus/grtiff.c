...
#ifdef OPUS_X64
/* The native entry point is implemented in C++.  Compile the original C
 * decoder under a private name so that the translation can forward to the
 * reference algorithm instead of duplicating its stream semantics. */
#undef HCREADTIFFLINE
#undef ReadTIFFLine
#define ReadTIFFLine C_ReadTIFFLine
#define sxt C_TiffSxt
#endif
