...
#ifdef OPUS_X64
	/*
	 * The 16-bit compiler treated sz[] as a zero-width initializer slot
	 * followed by the fixed descriptor fields.  A native pointer preserves
	 * the source descriptors without relying on that obsolete extension.
	 */
	const CHAR *sz;
	int     x;          /* idrb */
	int     y;          /* idcb/tmc */
...
#ifdef OPUS_X64
	/* Largest checked-in icon bar has fewer than 32 item descriptors. */
	struct IBID rgibid[32];
#else
	struct IBID rgibid[];
#endif
	};
...
struct IBID /* Icon Bar Item Description */
	...
	int     iibb : 8;   /* item index */
#ifdef OPUS_X64
	const CHAR *sz;     /* native replacement for the Win16 sz[] slot */
#else
	CHAR    sz[];       /* text */
#endif
	int     x;          /* idrb */ 
...
struct IBS /* Icon Bar Structure */
	...
	PFN     pfn;
#ifdef OPUS_X64
	UINT_PTR wRef;       /* pointer-sized user data */
#else
	WORD    wRef;       /* user data */
#endif
	int     ibc : 4;
...
#ifdef OPUS_X64
NATIVE UINT_PTR WRefFromHwndIb();
#else
...
#ifdef OPUS_X64
#define HibsFromHwndIb(hwnd) ((HIBS)GetWindowLongPtr((hwnd),IDWIBHIBS))
#else
