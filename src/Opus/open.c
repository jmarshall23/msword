...
struct RC * prcMw;    /* normally NULL; contains RC for new mw; used by CBT */
{
	...
	if (vmerr.fMwFull)
		{
#ifdef OPUS_X64
		OpusX64Trace("FCreateMw: MWD table full before allocation");
#endif
		goto FailExit;
		}
	...
	if (vmerr.fMemFail)
		{
#ifdef OPUS_X64
		OpusX64Trace("FCreateMw: MWD allocation failed");
#endif
		goto FailExit;
		}
	...
	if ((hwndMw = HwndCreateWindowRc( szClsMwd, ws, rc,
		...
		{
#ifdef OPUS_X64
		OpusX64Trace("FCreateMw: MWD window creation failed");
#endif
		goto FailExit;
		}
...
*/
		...
		if (hMwMenu == NULL)
			{
#ifdef OPUS_X64
			OpusX64Trace("FCreateMw: GetSystemMenu failed");
#endif
			goto FailExit;
			}
		...
		if (!ChangeMenu(hMwMenu, SC_RESTORE, 
			...
			{
#ifdef OPUS_X64
			OpusX64Trace("FCreateMw: system-menu conversion failed");
#endif
			goto FailExit;
			}
		...
		if ((w=GetMenuState(hMwMenu, 0, MF_BYCOMMAND)) != 0 && w != -1)
			{
#ifdef OPUS_X64
			OpusX64Trace("FCreateMw: separator command conversion failed");
#endif
			goto FailExit;
			}
		}
		...
		*/
		if (vwWinVersion >= 0x0300)
			if (!ChangeMenu(hMwMenu, SC_TASKLIST, 
				...
				{
#ifdef OPUS_X64
				OpusX64Trace("FCreateMw: Next Window system-menu conversion failed");
#endif
				goto FailExit;
				}
...
/* Create pane */
	...
	if ((wwActive = WwCreate(doc, wwNil, mw, rcTopPane, wk)) == wwNil)
		...
		{
#ifdef OPUS_X64
		OpusX64Trace("FCreateMw: WwCreate failed");
#endif
		goto FailExit;
		}
...
/* Create vertical scroll bar, split box and size box (if no horz scrl bar)*/
	...
	if (!FCreateScrollBars( &fVert, &fHorz, wwActive, *hmwd, rc, rcTopScrlBar,
		...
		{
#ifdef OPUS_X64
		OpusX64Trace("FCreateMw: scrollbar creation failed");
#endif
		goto FailExit;
		}
...
/* Create vertical scroll bar, split box and size box (if no horz scrl bar) */
	...
	if (*pfVert)
		...
		if ((pmwd->hwndSplitBox = HwndCreateWindowRc(szClsRSB,
			...
			{
#ifdef OPUS_X64
			OpusX64Trace("FCreateScrollBars: split-box window failed");
#endif
			goto LFailVert3;
			}
		...
		if (!FCreateVScrlWw(wwTop, rcTopScrlBar))
			{
#ifdef OPUS_X64
			OpusX64Trace("FCreateScrollBars: top vertical scrollbar failed");
#endif
			goto LFailVert2;
			}
...
LFailHorz:
#ifdef OPUS_X64
			OpusX64Trace("FCreateScrollBars: horizontal scrollbar failed");
#endif
			*pfHorz = fFalse;
			MeltHp();
			return (fFalse);
	...
	if (pmwd->fHorzScrollBar && pmwd->fVertScrollBar)
		...
		if ((pmwd->hwndSizeBox = HwndCreateWindowRc( szClsRSB,
			...
			{ /* failed; destroy horz scroll so AdjustMwdArea doesn't
				do weird things */
#ifdef OPUS_X64
			OpusX64Trace("FCreateScrollBars: size-box window failed");
#endif
...
LFail:
#ifdef OPUS_X64
	OpusX64Trace("FCreateVScrlWw: vertical scrollbar or page-view controls failed");
#endif
