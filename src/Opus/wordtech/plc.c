...
#if defined(MAC) || defined(OPUS_X64) /* C reference body replaces fetchn.asm on x64 */
/* D C P  A D J U S T */
...
#if defined(MAC) || defined(OPUS_X64) /* C reference body replaces editn.asm on x64 */
/* A D J U S T  H P L C */
			...
			pedl--;
				...
				int idr;
					...
					pdr = PdrFetch(pedl->hpldr, idr, &drfFetch);
							...
							pdr->cpFirst += dcp;
							#ifdef OPUS_X64
							C_AdjustHplcedlCps(pdr->hplcedl, cp, dcp);
							#else
...
#ifdef OPUS_X64
/* The Win16 public name was supplied by editn.asm while the C body used the
 * C_ prefix.  Preserve both entry points in the flat build. */
int AdjustHplcedlCps(hplcedl, cp, dcp)
struct PLCEDL **hplcedl;
CP cp;
CP dcp;
{
	C_AdjustHplcedlCps(hplcedl, cp, dcp);
	return 0;
}
#endif

#if defined(MAC) || defined(OPUS_X64) /* C reference body replaces resident.asm on x64 */
/* Q  I N  P L C */
