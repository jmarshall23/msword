...
#endif /* WIN */
{
	...
	/* The 16-bit Microsoft C compiler allowed a two-byte structure to be
	 * converted directly to int.  Modern MSVC does not; the union's integer
	 * view is the exact value that conversion represented. */
#ifdef OPUS_X64
	if (brc.brc == brcNone && fFrameLines)
#else
	if ((int) brc == brcNone && fFrameLines)
#endif
		return dzp;
#ifdef OPUS_X64
	if (brc.brc == brcNone || brc.brc == brcNil)
#else
...
#ifdef OPUS_X64
	rcl.ptTopLeft.xl = rcl.ptTopLeft.yl = 0;
#else
	rcl.ptTopLeft = 0L;
#endif
	RclToRcw(hpldr,&rcl,&rcw);
