...
/* store large grpprl in a PRC and return new PRM value */
	prc.bprlMac = cbLeft;
	return(PrmFromPrc(&prc, prmPrev.prm));
}
...
/* Check newly created prc to see if same as any of previous prcs stored in
	...
	/* allocate a new hprc */
#ifdef OPUS_X64
	cwRequest = CwFromCch(pprcNew->bprlMac + cbPRCBase);
#else
...
struct	PRC **hprc;
{
	...
	cfgrPrc = OpusPrcTokenFromHandle((void **)hprc);
	Assert(cfgrPrc != cfgrNil);
	pprm->cfgrPrc = cfgrPrc;
#else
#ifdef MAC
