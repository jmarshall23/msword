...
struct CLSE dncls[clsMax] =
	{ 
#ifdef OPUS_X64
	{ (void ***)mpdochdod, &docMac, docMax, cwDOD, (int)offset(MERR, fDocFull) },
	
		{ (void ***)mpfnhfcb, &fnMac, fnMax, cwFCB, (int)offset(MERR, fFnFull) },
	
		{ (void ***)mpwwhwwd, &wwMac, wwMax, cwWWD, (int)offset(MERR, fWwFull) },
	
		{ (void ***)mpmwhmwd, &mwMac, mwMax, cwMWD, (int)offset(MERR, fMwFull) }
#ifdef WIN
		,{ (void ***)mpdclhdcld, &dclMac, dclMax, cwDCLD, (int)offset(MERR, fDclFull) }
#endif
#else
...
#ifdef OPUS_X64
	Assert(*(struct PLC ***)((CHAR *)(*hdod) + edc * sizeof(int)) == hNil);
#else
...
#ifdef OPUS_X64
	*(struct PLC ***)((CHAR *)(*hdod) + edc * sizeof(int)) = hplc;
#else
	*((int *)(*hdod) + edc) = hplc;
#endif
