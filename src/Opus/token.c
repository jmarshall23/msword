...
#ifdef OPUS_X64
/* CS stored variable-sized counted strings inline after each token record.
 * Native C cannot form an array of structures with flexible members, so the
 * generated keyword list becomes a table of immutable C-string pointers. */
typedef struct _tkr
	{
	ELT elt;
	const CHAR *st;
...
CHAR * st;
{
	...
	while (--ditkr >= 0)
		{
#ifdef OPUS_X64
		if (cchOrg == (int)strlen(csrgtkr[itkr].st))
			{
			pch = st+2;
			lpch = csrgtkr[itkr].st+1;
#else
			...
			*pchCurtok = ' ';
			...
			/* FALL THROUGH */
				...
				case elccElt:
						...
						Assert(cch <= cchIdentMax);
						#ifdef OPUS_X64
						bltbx(lpst, (char far *) pchCurtok + 1, cch);
						#else
