...
#ifdef OPUS_X64
LRESULT CALLBACK StatLineWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
#else
		...
		if (vsci.fWin3)
			...
			if (vcchStatLinePrompt) {
				...
				if (vsci.fWin3Visuals)
					...
					dxpMin = rgslm[isiSep13].dxpOff + dxpSepGray2;
					dxp = rgslm[isiSep23].dxpOff - dxpMin;
					PatBlt( hdc, dxpMin, 2, dxp, 1, PATCOPY );
					...
					for( isi = isiExBl3; isi < isiLast3; isi += 2 ) {
						dxpMin = rgslm[isi].dxpOff - dxpSepToggleEnd;
						dxp = rgslm[isi].dxpFld + dxpSepToggleEnd2;
						PatBlt( hdc, dxpMin, 2, dxp, 1, PATCOPY );
...
#ifdef OPUS_X64
csconst char stForHelpPressF1[] = "\022For Help, press F1";
#else
