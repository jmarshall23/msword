...
struct CBM
	...
	BCM bcm;
#ifdef OPUS_X64
	/* MKCMD emits counted strings inline.  Give each generated record a
	 * fixed-width flat-memory representation instead of the 16-bit
	 * compiler's zero-length trailing array extension. */
	struct
		{
		CHAR cch;
		CHAR text[cchMaxSzMenu];
		} st;
#else
	CHAR st[];
#endif
...
int i;
{
	...
	for (;;)
		...
		LpstToSz((CHAR FAR *)&rgcbm[icbm++].st, rgch);
