...
#ifdef OPUS_X64
LRESULT CALLBACK RSBWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
#else
...
#endif
{
	...
	case WM_CREATE:
		if (vhwndCBT)
			...
			SendMessage(vhwndCBT, WM_CBTNEWWND, (WPARAM)hwnd, 0L);
...
/* handle hits everywhere except the thumb */
		for ( ;; )
			...
			if (GetWindowWord(hwnd, offset(RSBS,izppInvert)) != iNil)
				{
				SendMessage( hwndParent, msg,
						MAKEWPARAM(mpizppsbm [izpp], 0), (LPARAM)hwnd);
...
/* handle thumb dragging */
		...
		if (ZpFromPt(ptInvert) != ZpFromPt(ptHit))
			...
			SendMessage( hwndParent, msg,
					MAKEWPARAM(SB_THUMBPOSITION, spsT), (LPARAM)hwnd);
			};
		}
	ReleaseDC( hwnd, hdc );
	ReleaseCapture();
...
#ifdef OPUS_X64
	/* Win16 exposed the stock scrollbar bitmaps through DISPLAY.DRV.  On
	 * Win32/Win64 the same OBM_* resources are selected by passing a null
	 * module handle to LoadBitmap. */
	hDisplay = NULL;
#else
