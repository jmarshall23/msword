...
LIev3:
	...
	case 48: /* 48: <texp> -> <exp> */
		#ifdef OPUS_X64
		cgfiX64.cDigBlwDec = pev[1].cDigBlwDec;
		cgfiX64.gfi = pev[1].gfi;
...
EV	*pevTop;
{
	...
	for ( ; iRow <= iRowLast && cpCur < cpLim; iRow++)
		...
		for ( ; iCol <= iColLast; iCol++)
			...
			#ifdef OPUS_X64
			cDigBlw = avCur.cDigBlw;
			gfi = avCur.gfi;
			if (FNumFromCps(ptr2->doc, vtcc.cpFirst, vtcc.cpLim,
					&avCur.num, ppes, fFalse, &cDigBlw, &gfi))
			#else
			if (FNumFromCps(ptr2->doc, vtcc.cpFirst, vtcc.cpLim,
