...
WORD	wParam;
{
	...
	case tmmCreate:
		Assert(hwndStaticEdit == NULL);
		hInst = (HANDLE) GetWindowLongPtr(hwndDlg, GWLP_HINSTANCE);
		hwndStaticEdit = CreateWindow(
		...
		if (FNoHeap((struct UTXD **) GetWindowLongPtr(hwndStaticEdit, IDSEDITDSCRP)))
			{
			DestroyWindow( hwndStaticEdit );
			return fFalse;
			}

		SetWindowLongPtr(hwndStaticEdit, GWLP_WNDPROC,
				(LONG_PTR) lpprocStaticEditWndProc);
		break;
	case tmmPaint:
		Assert(hwndStaticEdit != NULL);
...
LONG		lParam;
{
	...
	hutxd = (struct UTXD **) GetWindowLongPtr(hwnd, IDSEDITDSCRP);
	...
	case WM_CREATE:
			...
			SetWindowLongPtr(hwnd, IDSEDITDSCRP, (LONG_PTR) NULL);
			...
			SetWindowLongPtr(hwnd, IDSEDITDSCRP, (LONG_PTR) hutxd);
			if (FNoHeap(hutxd))
				{
				...
				if (!FIsDlgDying())
					...
					if ((hwndT = GetParent(hwnd)) != NULL)
					    ...
					    if (hdlgSave != hdlgNull)
							...
							EnableTmc((TMC) GetDlgCtrlID(hwnd),
								(*hutxd)->stText[0]);
							}
		...
		if (!FIsDlgDying())
			...
			if ((hwndT = GetParent(hwnd)) != NULL)
			    ...
			    if (hdlgSave != hdlgNull)
					{
					hdlgSave = HdlgSetCurDlg(hdlgSave);
					SetFocusTmc((TMC) GetDlgCtrlID(hwnd));
					}
				HdlgSetCurDlg(hdlgSave);
				}
			}
