...
#ifdef OPUS_X64
	/* The 16-bit compiler allowed an integer to be treated as a BRC lvalue.
	 * Preserve the on-disk bit encoding without relying on native bitfield
	 * width or the removed struct-cast extension. */
	brc = (brc & ~brcDxpSpace) |
			((dptBrcSpace << shiftDxpSpaceBrc) & brcDxpSpace);
#else
...
#ifdef OPUS_X64
static csconst CHAR stAuto[] = { 4, 'A', 'u', 't', 'o' };
#else
static csconst CHAR stAuto[] = StKey("Auto",AutoDef);
#endif
...
#ifdef OPUS_X64
static csconst ENTBL rgEntbl[] = {
	{ cCharColor, {
		"\004Auto", "\005Black", "\004Blue", "\004Cyan", "\005Green",
		"\007Magenta", "\003Red", "\006Yellow", "\005White"
	} },
	{ cBrcp, {
		"\004None", "\003Box", "\003Bar", "\005Above", "\005Below"
	} },
	{ cBrcl, {
		"\004None", "\006Single", "\005Thick", "\006Double", "\006Shadow"
	} }
};
#else
static csconst ENTBL rgEntbl[] = {
