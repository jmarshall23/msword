...
#ifdef OPUS_X64
typedef union /* MaTh Comparison value: returned in one historical DWORD */
	{
	struct { SHORT w1, w2; };
	LONG l;
	} MTC;
...
#define PvalOfPahr(pahr,cas)	((char *)(pahr) + sizeof(struct AHR) +	\
					(cas) * sizeof(AS))
#ifdef OPUS_X64
#define PahrOfAd(ad) ((struct AHR *)*(void **)(uintptr_t)(ad))
#define HpahrOfAd(ad) ((struct AHR huge *)PahrOfAd(ad))
#define HpahrOfPahr(pahr) ((struct AHR huge *)(pahr))
#else
...
#ifdef OPUS_X64
/* A string descriptor becomes the address of its flat counted-string block.
 * The original WORD value was an index into sbStrings' selector table. */
typedef uintptr_t SD;
#define sdNil ((SD)UINTPTR_MAX)
#define CchFromSd(sd) (*(WORD *)(uintptr_t)(sd))
#define HpchFromSd(sd) ((BYTE *)((WORD *)(uintptr_t)(sd) + 1))
#else
typedef WORD SD;
#define sdNil	0xffff
...
*/
#ifdef OPUS_X64
#define HpeldiFromHeldi(heldi) ((ELDI huge *)*(void **)(heldi))
#else
...
struct ELCR
	...
	BYTE elcc;
	union	{
		/* These four records share the same counted-string layout.  The
		 * original compiler allowed four anonymous union variants to expose
		 * the same cch member; modern MSVC rejects the duplicate names. */
		struct	{
			BYTE cch;
			union {
...
#ifdef OPUS_X64
#define HpeliOfHeli(heli) ((ELI huge *)*(void **)(heli))
#else
