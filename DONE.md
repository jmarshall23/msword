...
`src/port/original/opus-asm-resn2-sttb.c`. The conversion keeps the exported C
ABI entry points for `HpstFromSttb`, `PstFromSttb`, `FStcpEntryIsNull`,
`GetStFromSttb`, `IbstFindSt`, and `OpusValidateSttb`, including the invalid
STTB trace messages and stack-frame reporting path.

The replacement drops C++ headers, namespace scope, `extern "C"`,
`static_cast`, `reinterpret_cast`, `nullptr`, and `auto`, and keeps the target
source name hyphenated for the new C file.

Validated with `cmake --build out/macos-debug --target opus_x64_runtime
opus_original_sttb_test -j2`, `ctest --test-dir out/macos-debug -R
'^opus_original_sttb_test$' --output-on-failure`, `ctest --test-dir
out/macos-debug -R '^opus_x64_runtime_test$' --output-on-failure`, `ctest
--test-dir out/macos-debug -L ui --output-on-failure`, the C++ surface grep
over `src/port/original/opus-asm-resn2-sttb.c`, the `nm` symbol check in
`libopus_x64_runtime.a`, and `git diff --check`.
...
conversion keeps the same return-code checks across class registration,
windows, text, DCs, clipboard, child enumeration, paint, geometry, input,
timers, menus, properties, and destruction while replacing the anonymous
namespace, `constexpr`, C++ casts, `nullptr`, C++ aggregate initialization, and
`std::memcpy` with C equivalents.

Validated with `cmake --build build-chrstride --target opus_win32_user32_test
-j2`, `ctest --test-dir build-chrstride -R '^opus_win32_user32_test$'
--output-on-failure`, the broader `ctest --test-dir build-chrstride -R
'strtbl|sttb|plc|sdm_cab|command|opus_x64_runtime_test|win32_memory|opus_win16_module_test|opus_win32_resource_test|opus_win32_gdi_(object|raster)_test|opus_win32_font_test|opus_win32_print_test|opus_win32_user32_test|win32_coverage|word1_port_smoke_test|word1_scripted_key_test|opus_word1_typing_test|opus_word1_clipboard_shortcut_test|opus_word1_unicode_test|opus_word1_about_test|opus_word1_selection_test|opus_word1_interaction_test|opus_word1_save_as_test|opus_word1_pdf_export_test|opus_word1_font_typing_test'
--output-on-failure` sweep, and `git diff --check`.
...
## Convert font test to C11

`opus_win32_font_test` now compiles from a C11 source instead of C++20. The
conversion keeps the same return-code checks, font enumeration checks, and
golden TSV metric validation while replacing `std::array`, streams, strings,
vectors, `std::stoi`, C++ casts, `nullptr`, and C++ aggregate initialization
with bounded C file parsing and fixed arrays.

Validated with `cmake --build build-chrstride --target opus_win32_font_test
-j2`, `ctest --test-dir build-chrstride -R '^opus_win32_font_test$'
--output-on-failure`, the broader `ctest --test-dir build-chrstride -R
'strtbl|sttb|plc|sdm_cab|command|opus_x64_runtime_test|win32_memory|opus_win16_module_test|opus_win32_resource_test|opus_win32_gdi_(object|raster)_test|opus_win32_font_test|opus_win32_print_test|opus_win32_user32_test|win32_coverage|word1_port_smoke_test|word1_scripted_key_test|opus_word1_typing_test|opus_word1_clipboard_shortcut_test|opus_word1_unicode_test|opus_word1_about_test|opus_word1_selection_test|opus_word1_interaction_test|opus_word1_save_as_test|opus_word1_pdf_export_test|opus_word1_font_typing_test'
--output-on-failure` sweep, and `git diff --check`.
...
## Convert memory test to C11

`opus_win32_memory_test` now compiles from a C11 source instead of C++20. The
conversion keeps the same return-code checks and target linkage while replacing
C++ casts, `nullptr`, C++ zero initialization, `auto`, and `std::` C library
calls with plain C equivalents. Typed local aliases keep the legacy Ansi helper
calls warning-clean without changing the shared Win32 header.

Validated with `cmake --build build-chrstride --target
opus_win32_memory_test -j2`, `ctest --test-dir build-chrstride -R
'^opus_win32_memory_test$' --output-on-failure`, the broader `ctest --test-dir
build-chrstride -R
'strtbl|sttb|plc|sdm_cab|command|opus_x64_runtime_test|win32_memory|opus_win16_module_test|opus_win32_resource_test|opus_win32_gdi_(object|raster)_test|opus_win32_font_test|opus_win32_print_test|opus_win32_user32_test|win32_coverage|word1_port_smoke_test|word1_scripted_key_test|opus_word1_typing_test|opus_word1_clipboard_shortcut_test|opus_word1_unicode_test|opus_word1_about_test|opus_word1_selection_test|opus_word1_interaction_test|opus_word1_save_as_test|opus_word1_pdf_export_test|opus_word1_font_typing_test'
--output-on-failure` sweep, and `git diff --check`.
...
sources instead of C++20. The conversion keeps the same assertions and target
linking while replacing `nullptr`, C++ casts, `auto`, `using`, `std::array`,
and range-for with plain C typedefs, casts, arrays, and loops.

Validated with `cmake --build build-chrstride --target opus_win16_module_test
opus_win32_resource_test -j2`, `ctest --test-dir build-chrstride -R
'^opus_win16_module_test$|^opus_win32_resource_test$'
--output-on-failure`, the broader `ctest --test-dir build-chrstride -R
'strtbl|sttb|plc|sdm_cab|command|opus_x64_runtime_test|win32_memory|opus_win16_module_test|opus_win32_resource_test|opus_win32_gdi_(object|raster)_test|opus_win32_font_test|opus_win32_print_test|opus_win32_user32_test|win32_coverage|word1_port_smoke_test|word1_scripted_key_test|opus_word1_typing_test|opus_word1_clipboard_shortcut_test|opus_word1_unicode_test|opus_word1_about_test|opus_word1_selection_test|opus_word1_interaction_test|opus_word1_save_as_test|opus_word1_pdf_export_test|opus_word1_font_typing_test'
--output-on-failure` sweep, and `git diff --check`.
...
startup loop, queues the OK command for File/New's modal dialog, sends
File/New, verifies the main caption advances to `Document2`, then sends
File/Exit through the original command path. The final `ui` test label block is
gone, so `ctest -L ui` is empty.

Validated with `cmake --build build-chrstride --target WORD1 -j2`,
`ctest --test-dir build-chrstride -R '^opus_word1_ui_test$'
...
## Replace WORD1 font typing UI harness

`opus_word1_font_typing_test` now runs `WORD1 --scripted-font-typing-test` in
process with `OPUS_HEADLESS=1 SDL_VIDEODRIVER=dummy`. The scripted mode exits
the normal startup loop, drives the visible toolbar font and size combos
through their existing `CBN_SELCHANGE`/`CBN_SELENDOK` forwarding path, inserts
text through Word's diagnostic Unicode path, and fails unless the selected
`ftc`/`hps` values survive into typed runs, formatter line heights, selection
resizing, and the list-committed 72-point resize.

Validated with `cmake --build build-chrstride --target WORD1 -j2`,
`ctest --test-dir build-chrstride -R '^opus_word1_font_typing_test$'
--output-on-failure`, the scripted group
`ctest --test-dir build-chrstride -R '^opus_word1_font_typing_test$|^opus_word1_pdf_export_test$|^opus_word1_save_as_test$|^opus_word1_interaction_test$|^opus_word1_selection_test$|^opus_word1_about_test$|^opus_word1_unicode_test$|^opus_word1_clipboard_shortcut_test$|^opus_word1_typing_test$|^word1_scripted_key_test$'
--output-on-failure`, the broader
`ctest --test-dir build-chrstride -R 'strtbl|sttb|plc|sdm_cab|command|opus_x64_runtime_test|win32_memory|opus_win16_module_test|opus_win32_resource_test|opus_win32_gdi_(object|raster)_test|opus_win32_font_test|opus_win32_print_test|opus_win32_user32_test|win32_coverage|word1_port_smoke_test|word1_scripted_key_test|opus_word1_typing_test|opus_word1_clipboard_shortcut_test|opus_word1_unicode_test|opus_word1_about_test|opus_word1_selection_test|opus_word1_interaction_test|opus_word1_save_as_test|opus_word1_pdf_export_test|opus_word1_font_typing_test'
--output-on-failure` sweep, `ctest --test-dir build-chrstride -N -L ui`
showing 1 remaining UI-labelled test, and `git diff --check`.

Reviewed by agy and claude: agy could not start because its TTY UI failed to
open `/dev/tty`; claude stalled without output and was stopped.

## Replace WORD1 PDF export UI harness

`opus_word1_pdf_export_test` now runs `WORD1 --scripted-pdf-export-test` in
process with `OPUS_HEADLESS=1 SDL_VIDEODRIVER=dummy`. The scripted mode exits
the normal startup loop, inserts the same PDF test text through Word's
diagnostic Unicode path, exports through the Win95 shell command, verifies
Word's PDF export stage reaches success, and reads the generated file's
`%PDF-` header before deleting it.

Validated with `cmake --build build-chrstride --target WORD1 -j2`,
`ctest --test-dir build-chrstride -R '^opus_word1_pdf_export_test$'
--output-on-failure`, the scripted group
`ctest --test-dir build-chrstride -R '^opus_word1_pdf_export_test$|^opus_word1_save_as_test$|^opus_word1_interaction_test$|^opus_word1_selection_test$|^opus_word1_about_test$|^opus_word1_unicode_test$|^opus_word1_clipboard_shortcut_test$|^opus_word1_typing_test$|^word1_scripted_key_test$'
--output-on-failure`, the broader
`ctest --test-dir build-chrstride -R 'strtbl|sttb|plc|sdm_cab|command|opus_x64_runtime_test|win32_memory|opus_win16_module_test|opus_win32_resource_test|opus_win32_gdi_(object|raster)_test|opus_win32_font_test|opus_win32_print_test|opus_win32_user32_test|win32_coverage|word1_port_smoke_test|word1_scripted_key_test|opus_word1_typing_test|opus_word1_clipboard_shortcut_test|opus_word1_unicode_test|opus_word1_about_test|opus_word1_selection_test|opus_word1_interaction_test|opus_word1_save_as_test|opus_word1_pdf_export_test'
--output-on-failure` sweep, `ctest --test-dir build-chrstride -N -L ui`
showing 2 remaining UI-labelled tests, and `git diff --check`.
