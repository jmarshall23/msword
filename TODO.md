...
`opus_word1_ui_test` build now advances through the display/create/edit dialog
frontier, including the remaining `disp1.c` lvalue-cast resets, legacy early returns
...
Its DOCX-open caption probe now reads `GetWindowTextW` into `WCHAR` storage and
checks the fixed `Document1` text through `OPUSW`.
Its font-typing combo probes now pass fixed `CB_FINDSTRINGEXACT` text through
`OPUSW` storage.
Its ASCII `post_keyboard_character` path now uses `WCHAR`/`OPUSW` scalars, with
explicit casts at the remaining host `wchar_t` loop boundaries.
