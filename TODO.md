...
Items 1 through 4 are in strict order; each blocks the next at configure time. Item 5
lands once item 4 gives it something to run. One item from the next phase reaches back
into this one: item 5a's SDL decision has to be made before item 4 writes the presets,
...
compile and run under clang and gcc before anything else can be attempted.
